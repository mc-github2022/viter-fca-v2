...
const LearningTypeList = ({ setItemEdit }) => {
  ...
  return (
    <>
      ...
      <div className="datalist max-w-[650px] w-full overflow-x-hidden overflow-y-auto h-[450px] lg:max-h-[580px] custom__scroll  poco:max-h-[640px] lg:poco:max-h-[400px] relative">
        ...
        {!isLoading && learningtype.success === false ? (
          ...
          learningtype?.data.map((item, key) => (
            ...
            >
              <div
                className={`${
                  item.learning_type_active ? "opacity-100" : "opacity-40"
                } `}
              >
                <p className="mb-1">{item.learning_type_name}</p>
              </div>

              <ul className="datalist__action flex items-center gap-1 pr-3 ">
                {item.learning_type_active === 1 ? (
                  <>
                    <li className=" ">
                      <button
                        className="tooltip"
      ...
      {store.isSettingConfirm && (
        <ModalConfirm
          ...
          item={dataItem.learning_type_name}
          queryKey={"learningtype"}
          isArchive={isArchive}
        />
      )}
...
export default LearningTypeList;
