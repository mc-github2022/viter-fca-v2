...
class UserSystem
{
    ...
    public function search()
    {
        try {
            ...
            $sql .= "or user.user_system_email like :user_system_email ";
            $sql .= "or CONCAT(user_system_fname, ' ', user_system_lname) like :fullname ";
            $sql .= ") ";
            $sql .= "order by user.user_system_is_active desc, ";
            $sql .= "user.user_system_fname asc ";
            $query = $this->connection->prepare($sql);
            $query->execute([
                ...
                "fullname" => "%{$this->user_system_search}%",
            ]);
        } catch (PDOException $ex) {
            $query = false;
        }
